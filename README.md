...
Playwright 기반 Threads 수집, 중복 제거, 규칙 기반 평가를 통해 AI 정보를 "오늘 볼 것", "나중에 볼 것", "검토 필요"로 정리하는 개인용 로컬 대시보드입니다.

사용자가 로그인한 Chrome 프로필을 재사용하며, 수집 결과는 로컬 H2 파일 DB에 저장됩니다.

현재 MVP에서는 Threads를 핵심 수집 경로로 사용합니다.

## 0. 설계 목표
이 프로젝트는 AI를 이용해 사용자가 검토해야 하는 정보량을 줄이는 것을 목표로 설계했습니다.

따라서 자동 평가보다 사용자의 최종 판단을 우선하며, 사용자가 직접 변경한 상태는 재평가 과정에서도 덮어쓰지 않도록 만들어졌습니다.

또한 개인용 로컬 도구라는 목표에 맞춰 상시 서버 운영이나 자동 수집 대신 H2 파일 DB와 수동 수집 방식을 유지하고 있습니다.
...
- H2 파일 DB는 개인용 로컬 MVP를 전제로 선택했으며, 다중 사용자 환경이나 운영 배포 환경은 고려하지 않았습니다.
...
개인용 로컬 도구를 넘어서는 경우 사용자 인증, 비밀정보 관리, 배포 전략을 별도로 설계할 예정입니다.
